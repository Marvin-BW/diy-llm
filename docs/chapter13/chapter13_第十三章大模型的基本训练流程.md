...
### 人类强化反馈强化学习（Reinforcement Learning from Human Feedback, RLHF）
...
## 13.5 RLHF中常用的强化学习算法：PPO（Proximal Policy Optimization，近端策略优化）算法
...
| **奖励模型** $r_\phi$ | 给生成的回复打分 |  否，冻结 |
| **旧策略** $\pi_{\text{old}}$ | 用于重要性采样的“旧”副本，每轮采样后更新为当前策略 |  否，仅用于计算重要性权重 |
