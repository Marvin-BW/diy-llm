...
>因此，在实际大规模训练系统中，PP通常作为一种补充方案，与DP和TP结合使用：在节点内利用高速互联执行TP，在节点间通过PP降低跨节点通信开销，从而在内存、通信与并行效率之间取得平衡。
...
>在TPU、GPU集群（如NVIDIA H100）中，尽管GPU集群的互联带宽通常低于TPU Pod，但在实践中仍以DP和TP为主。相比之下，*PP模型按层划分为多个阶段串行执行，其并行粒度较粗，并在不同阶段之间引入前后依赖关系，限制了整体计算的并行度，而“气泡”现象是这种依赖带来的具体表现之一*。
