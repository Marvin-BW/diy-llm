...
#### 1. FlashAttention V2 的核心改进：交换循环顺序
...
#### 1. FlashAttention V3 的核心改进在异步 WGMMA 流水线
...
**矩阵乘法 $QK^T$** 使用 FP8 执行，充分利用 Tensor Core 的高吞吐，**累加器** 保持 FP16 或 BF16 精度，避免 FP8 累加时的精度损失。**Softmax 计算** 提升到 FP32 进行，保证指数运算的数值稳定性（softmax 中的指数和除法极易在低精度下溢出）。同时**输出 $PV$** 可选择性转换为 FP8，以适应后续层。
